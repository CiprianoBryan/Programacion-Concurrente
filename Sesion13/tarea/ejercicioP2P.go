...
func manejadorHP(con net.Conn) {
	...
	bufferIn := bufio.NewReader(con)
	infoJsonBytes, _ := bufferIn.ReadString('\n')
	var infoJson Info
	json.Unmarshal([]byte(infoJsonBytes), &infoJson)

	fmt.Printf("Se recibió el contador: %d, con Suma: %d, N: %d\n", infoJson.Contador, infoJson.Suma, infoJson.Numero)
	infoJson.Suma += infoJson.Contador
	// lógica
	if infoJson.Contador == infoJson.Numero {
		fmt.Println(infoJson.Suma)
		fmt.Printf("Finaliza el proceso con la suma: %d\n", infoJson.Suma)
	} else {
		infoJson.Contador += 1
		enviarProximo(infoJson)
	}
}

func enviarProximo(infoJson Info) {
	// de forma aleatoria se va a seleccionar la identificacion del nodo
	indice := rand.Intn(len(bitacoraAddrHP))
	fmt.Printf("Enviando el contador %d al nodo %s\n", infoJson.Contador, bitacoraAddrHP[indice])
